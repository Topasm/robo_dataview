...
`POST /search/semantic` defaults to text embeddings over episode text and
...
limited to matching episodes, their annotations, and stored embedding rows from
matching episodes. Filtered semantic searches use a transient in-memory text
index so they do not overwrite the persisted full dataset embedding mirror.

Optional `modalities` and `source_model` fields can target stored non-text rows,
for example `{"modalities": ["image"], "source_model": "..."}` for visual
keyframes generated by a compatible model. `visual` expands to `image` and
`video_clip`; `all` includes text, image, video clip, and trajectory rows.
Cross-modal ranking is only meaningful when the query embedding provider and
stored visual embeddings use the same vector space.

The default provider is deterministic text hashing. Set
...
`ROBOT_DATA_STUDIO_EMBEDDING_TIMEOUT_SECONDS` as needed. Set
`ROBOT_DATA_STUDIO_EMBEDDING_PROVIDER=clip`, `siglip`, `transformers-text`, or
`visual-compatible` to use an optional Transformers text encoder with
`source_model=transformers-vision:<model>`, matching visual embedding rows
created with the same CLIP/SigLIP model. When `lancedb` is
