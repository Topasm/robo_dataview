...
class ExportServiceTest(unittest.TestCase):
    ...
    def test_lance_export_writes_subset_artifact(self) -> None:
        ...
        self.assertTrue(Path(artifact["files"]["state_body_stats"]).exists())
        self.assertTrue(Path(artifact["files"]["action_body_stats"]).exists())
        ...
        self.assertEqual(metadata["stats"]["source_table"], "train_episodes.lance")
        self.assertEqual(metadata["stats"]["source_row_unit"], "episode")
        self.assertEqual(metadata["stats"]["state_count"], 180)
        self.assertEqual(metadata["stats"]["action_count"], 180)
        self.assertEqual(artifact["materialized"]["state_stats_count"], 180)
        self.assertEqual(artifact["materialized"]["action_stats_count"], 180)
        self.assertEqual(artifact["validation"]["state_stats_count"], 180)
        self.assertEqual(artifact["validation"]["action_stats_count"], 180)
        stats = json.loads(Path(artifact["files"]["stats"]).read_text(encoding="utf-8"))
        state_stats = json.loads(
            Path(artifact["files"]["state_body_stats"]).read_text(encoding="utf-8")
        )
        action_stats = json.loads(
            Path(artifact["files"]["action_body_stats"]).read_text(encoding="utf-8")
        )
        self.assertEqual(max(stats["observation.state"]["count"]), 180)
        self.assertEqual(max(stats["action"]["count"]), 180)
        self.assertEqual(state_stats["feature"], "observation.state")
        self.assertEqual(action_stats["feature"], "action")
